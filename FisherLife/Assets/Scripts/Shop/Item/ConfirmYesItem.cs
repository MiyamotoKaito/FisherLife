...
{
    /// <summary>
    ///     確認パネルの「はい」行。渡された取引対象を実行して一つ前へ戻る。
    ///     取引不可(お金不足/在庫なし)のときは自身の画像を薄暗くする。
    /// </summary>
    public class ConfirmYesItem : ItemBase
    {
        ...
        /// <summary> 取引対象を渡す（ComfilmPanel.Setupから）。 </summary>
        public void SetTarget(TradeItem target) => _target = target;

        /// <summary> 取引可否に応じて自身の画像の明るさを切り替える。 </summary>
        ...
        public override async void OnEntry(ShoppingController controller)
        {
            ...
            controller.Pop(); // 取引後、確認を閉じてリストへ戻る
        }
